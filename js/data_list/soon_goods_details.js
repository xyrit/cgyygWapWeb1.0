...
var waitTime_2;//倒计时总数
var pri_last_attend_time;//用户最后参与时间

var glo_newlottery_time; //全局揭晓时间
...
function getGoodsList(){
	$.ajax({  
		...
		success:function(data){
			...
			var last_attend_time=prizeList['last_attend_time'];//用户的最后参与时间
			...
			if(Number(last_attend_time)>hourTime){
				//下一期的时间
			...
			//时间戳格式化
			  ...
			  glo_newlottery_time=newlottery_time; //传递全局 给开奖接口
			  //在倒计时走完前先写入轮播图
			 
			//alert(lottery_code);
			//alert(typeof(lottery_code));
			if(lottery_code!=10000000){
				  $("#title").prev().html("已揭晓"); //修改标题状态
				...
				//参与码要分割
				  ...
				  //填充-----获奖号码
				  var lukyCodeStr='';
				  lukyCodeStr+='获奖幸运码：<span class="fz_bl display-ib" id="lotteryCode">'+lottery_code+'</span>';
				  lukyCodeStr+='<a class="luckyCode-see display-ib" id="rcorners5" href="7_Ard-announced-Calculation.html">查看计算详情</a>';
				  
				  $(".winner-luckyCode").html(lukyCodeStr);
				  //构建用户信息结构
				  var lukyUser='';
				  lukyUser+='<div class="winner-record position-re display-b">';
				  lukyUser+='<div class="winner-icon position-ab display-ib">';
                  lukyUser+='</div>';       
                  lukyUser+='<div class="winner-list row">';     
				      
                  lukyUser+='<div class="col-md-2 col-sm-3 col-xs-4 winner_publish_1 text-center" id="userImg">';   
				  ...
				  if(typeof(uid) == "undefined"||uid!=uid_other){
					  ...
					  lukyUser+='<a href="35-personal-other.html?uid='+uid_link+'"><img src="'+p+'" alt="" class="userImg img_publish"></a>';  
					}else{
						lukyUser+='<a href="35-personal.html"><img src="'+p+'" alt="" class="userImg img_publish"></a>';   
						
					}	 
                  //lukyUser+='<img class="img_publish" src="images/tx.png">';      
				         
                  lukyUser+='</div>';  
				                 
                  lukyUser+='<div class="col-md-10 col-sm-9 col-xs-8 winner_publish_1 margin-t-5 padding-l-0">';  
				  lukyUser+='<p class="lg_22px" id="prizer">';     
				    
				  //点击昵称跳转到相应的个人中心页面
				  if(typeof(uid) == "undefined"||uid!=uid_other){
					  
					  lukyUser+='<a href="35-personal-other.html?uid='+uid_link+'" class="text-red">'+nickname+'</a>';
					  
					}else{
						
						lukyUser+='<a href="35-personal.html" class="text-red">'+nickname+'</a>';
					} 	
					    
                  
				  
				  lukyUser+='<span class="text-gray">（'+ip_address+'）</span></p>';
                  lukyUser+='<p class="lg_22px text-gray" id="userId">用户ID：'+uid_other+'（唯一不变标识）</p>';
                  lukyUser+='<p class="lg_22px"><span class="text-gray">本期参与：</span><span class="text-red" id="attendCount">'+attend_count+'</span><span class="text-gray">人次</span><a href="javascript:;" class="text-blue padding-l-r-5" onClick="alertList('+uid_other+','+lottery_id+');">查看TA的号码</a></p>';
                  lukyUser+='<p class="lg_22px text-gray" id="publishTime">揭晓时间：'+newlottery_time+'</p>';
                  lukyUser+='</div>';                    
                  lukyUser+='</div>';                    
                  lukyUser+='</div>';              
                     
                  $(".winner-luckyCode").before(lukyUser); 
			...
			//点击昵称跳转到相应的个人中心页面
			/*if(typeof(uid) == "undefined"||uid!=uid_other){
				$("#prizer").html('获奖者：<a href="35-personal-other.html?uid='+uid_link+'" class="text-red">'+nickname+'</a>（IP所在地址）');
			  }else{
				  
				  $("#prizer").html('获奖者：<a href="35-personal.html" class="text-red">'+nickname+'</a>（IP所在地址）');
			  }  
			
			$("#attendCount").html(attend_count);
			$("#publishTime").html('揭晓时间：'+lottery_time+'');
	...
	$.ajax({  
		...
		success:function(data){ 
		   ...
		   if(userInfo!=''){
			   ...
			   //传值传递给计算规则
			   window.sessionStorage.lottery_code  = (glo_startCode+lottery_code);//橙果中奖号码
			   ...
			   var last_attend_time=userInfo['last_attend_time'];//用户最后参与时间，传递查看计算详情
				...
				//填充-----获奖号码----替换倒计时
				  var lukyCodeStr='';
				  lukyCodeStr+='获奖幸运码：<span class="fz_bl" id="lotteryCode">'+(glo_startCode+lottery_code)+'</span>';
				  //点击 查看计算详情，传递用户最后参与时间过去查看计算详情页面，其他的如时时彩的号码等字段通过本地存储 传递
				  lukyCodeStr+='<a class="luckyCode-see" id="rcorners5" href="7_Ard-announced-Calculation.html?lastAttendTime='+last_attend_time+'">查看计算详情</a>';
				  $(".winner-luckyCode").html(lukyCodeStr);
				  //构建用户信息结构
				  var lukyUser='';
				  lukyUser+='<div class="winner-record position-re display-b">';
				  lukyUser+='<div class="winner-icon position-ab display-ib">';
                  lukyUser+='</div>';       
                  lukyUser+='<div class="winner-list row flex_content">';     
				      
                  lukyUser+='<div class="col-md-2 col-sm-3 col-xs-4 winner_publish_1 text-center padding-r-10" id="userImg">';   
                  ...
                  lukyUser+='<div class="col-md-10 col-sm-9 col-xs-8 winner_publish_1 margin-t-5 padding-l-0 padding-r-0 fz_12px">';  
				  lukyUser+='<p class="lg_22px" id="prizer">';     
				  ...
				  lukyUser+='（'+attend_ip+'）</p>';             
                  lukyUser+='<p class="lg_22px text-gray" id="userId">用户ID：'+uid_other+'（唯一不变标识）</p>';
                  lukyUser+='<p class="lg_22px"><span class="text-gray">本期参与：</span><span class="text-red" id="attendCount">'+prizeCount+'</span><span class="text-gray">人次</span><a href="javascript:;" class="text-blue padding-l-r-5" onClick="alertList('+uid_other+','+lottery_id+');">查看TA的号码</a></p>';
                  lukyUser+='<p class="lg_22px text-gray" id="publishTime">揭晓时间：'+newlottery_time+'</p>';
                  lukyUser+='</div>';                    
                  lukyUser+='</div>';                    
                  lukyUser+='</div>';              
                  ...
                  $(".winner-luckyCode").before(lukyUser);             
				  $("#title").prev().html("已揭晓"); 
				//$(".winner-luckyCode").prepend(lukyUser); 
			   ...
			   swal({
					  ...
					  confirmButtonColor: "#ff4800",
					  confirmButtonText: '等待超时，请刷新页面',//成功按钮
